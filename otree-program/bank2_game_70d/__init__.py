...
# --- bank1_game state pool ---
# Plug in the collected bank1_game states here before running the actual experiment.
# Each entry must be 'Low', 'Medium', or 'High'.
# Example: PREBANK_STATES = ['Low', 'Medium', 'High', 'Medium', ...]
# When non-empty, each group's prebank_state is drawn by random.choice() from this pool.
# When empty (default), falls back to the probability-based random draw (for demo/testing).
PREBANK_STATES = []
...
def creating_session(subsession: Subsession):
    ...
    for group in subsession.get_groups():
        if PREBANK_STATES:
            prebank_state = random.choice(PREBANK_STATES)
        else:
...
class SecondStage(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            ...
            delayed_a=10-C.DELAY_COST,
            unmatched_corr=(100-C.STATE_CORR)//2,
            other_withdrew=other.first_decision == 'action_a',
