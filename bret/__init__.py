...
def set_payoff(player: Player):
    ...
    if round_number == 1:
        participant.vars['round_to_pay'] = NUM_ROUNDS
    ...
    if round_number == NUM_ROUNDS:
        total_payoff_cents = sum(int(float(p.payoff)) for p in player.in_all_rounds())
        participant.bret_payoff_dollars = round(total_payoff_cents / 100, 2)
        participant.bret_boxes_collected = player.boxes_collected
        participant.bret_bomb = player.bomb
    ...
    def vars_for_template(player: Player):
        ...
        round_data_cents = [
            dict(
                ...
                boxes_collected=p.boxes_collected,
                bomb=p.bomb,
                round_result_cents=int(float(p.round_result)),
            )
            for p in all_rounds
        ]
        total_payoff_cents = sum(int(float(p.payoff)) for p in all_rounds)
        return dict(
            round_data_cents=round_data_cents,
            total_payoff_cents=total_payoff_cents,
            total_payoff_dollars=total_payoff_cents / 100,
            box_value=BOX_VALUE,
